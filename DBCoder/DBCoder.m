...
- (void)updateStatement:(DBStatement)statement
{
    ...
    NSUInteger keys_count = [data columnsCount];
...
- (DBCoder *)decoderForColumn:(NSString *)column withScheme:(id<DBScheme>)_scheme
{
    NSString *query = [NSString stringWithFormat:@"SELECT * FROM %@ WHERE %@ = ? LIMIT 1", [_scheme table], [_scheme primaryKeyColumn]];
    NSArray *decoders = [decodingService decodersWithScheme:_scheme fromSQLQuery:query withArgs:@[[self decodeObjectForColumn:column]]];
    return [decoders firstObject];
}
