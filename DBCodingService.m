...
- (void)saveOneToManyWithId:(id)objectId inCoder:(DBCoder *)coder
{
    ...
    for (NSString *foreignKey in allKeys)
    {
        ...
        if (objectsScheme) {
            ...
            NSString *pkColumn = [objectsScheme primaryKeyColumn];
            [self deleteOneToManyObjectsWithScheme:objectsScheme withForeignKey:foreignKey where:[NSString stringWithFormat:@"%@ = ? AND %@ NOT IN (%@)", foreignKey, pkColumn, idsPlaceholder] args:savedIdentifiers];
        }
